...
const Header = ({ onMenuClick }: HeaderProps) => {
  ...
  return (
    <header className="lg:hidden bg-background border-b border-border px-3 sm:px-4 py-2.5 sticky top-0 z-40 pt-safe-top">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <div className="flex flex-col">
            ...
            {displayRole && (
              <span className="text-[10px] text-muted-foreground leading-tight">{displayRole}</span>
            )}
          </div>
        </div>
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <button className="focus:outline-none rounded-full">
                <Avatar className="h-8 w-8 border-2 border-border cursor-pointer">
                  {avatarImageUrl && (
                    <AvatarImage 
                      src={avatarImageUrl}
                      alt={user?.name}
